...
fastify.register(cors, {
  origin: (origin, cb) => {
    // 1. Allow requests with no origin (mobile apps, curl, server-side)
    if (!origin) {
      cb(null, true);
      return;
    }

    // 2. Always allow our own dashboard
    const isInternal = allowedOrigins.some((allowed) => {
      try {
        return new URL(allowed).hostname === new URL(origin).hostname;
      } catch {
    ...
    // 3. For public API endpoints, allow any origin
    // This allows your customers to use their API keys from their own web apps
    cb(null, true);
  },
...
fastify.post('/classify', async (request, reply) => {
  ...
  try {
    ...
    await nsfwQueue.add(
      ...
      {
        attempts: 2,
        removeOnComplete: true, // ← wipe immediately after success

...
export function createForceField(
  ...
  function readPageLum(): number {
    try {
      ...
      // best-effort luminance probe; fall through to the default below
    }
    return 0;
  }
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        // capture is best-effort; skip this frame on failure
      }
    };
  }
  ...
  function makeTarget(width: number, height: number, emission = false, float = false): Target {
  ...
  function syncCanvasSize() {
    ...
    for (let i = MIPS - 1; i >= 0; i--) {
      const up =
        i === 0 ? { width: brightTarget.width, height: brightTarget.height } : downTargets[i - 1];
  ...
  function render() {
    ...
    gl!.uniform3f(fu.uEdgeColor, config.edgeColor[0], config.edgeColor[1], config.edgeColor[2]);
    ...
    gl!.uniform1f(fu.uGridRevealStrength, clamp(config.gridRevealStrength, 0, 3));
    gl!.uniform1f(fu.uGridRevealRadius, clamp(config.gridRevealRadius, 60, 800) * dpr);
    ...
    if (bloomOn) {
      ...
      gl!.uniform1f(brightPass.uniforms.uThreshold, clamp(config.bloomThreshold, 0, 1));
      drawQuad();
      ...
      for (let i = 0; i < MIPS; i++) {
        ...
        gl!.uniform2f(downPass.uniforms.uTexel, 0.5 / src.width, 0.5 / src.height);
        drawQuad();
        src = downTargets[i];
      }
      for (let i = MIPS - 1; i >= 0; i--) {
    ...
    gl!.uniform1f(compositePass.uniforms.uBloomStrength, bloomOn ? clamp(config.bloom, 0, 3) : 0);
  ...
  function frame(now: number) {
    ...
    for (let i = 0; i < MAX_HITS; i++) {
      if (hitTime[i] > -900 && time - hitTime[i] < config.rippleDuration + 0.3) {
        ripplesLive = true;
        break;
      }
    }
    const active = ripplesLive || now - lastInput < 500 || reducedMotion || contentDirty;
  ...
  return {
    ...
    destroy() {
      ...
      if (sceneTarget.emissionTexture) gl!.deleteTexture(sceneTarget.emissionTexture);
      ...
      for (const pass of [fieldPass, brightPass, downPass, upPass, compositePass]) {
...
export function ForceField({ children, className, style, ...options }: ForceFieldProps) {

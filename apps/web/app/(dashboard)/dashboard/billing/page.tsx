...
export default function DashboardBillingPage() {
  const [subscription, setSubscription] = useState(DUMMY_SUBSCRIPTION);
  const [loading, setLoading] = useState<string | null>(null);
  const [preview, setPreview] = useState<{ amount: number; currency: string; priceId: string; prorationDate: number } | null>(null);
  ...
  return (
    <div className="container mx-auto py-10 space-y-12 pb-20">
      <div className="space-y-4">
        <h1 className="text-3xl font-bold italic uppercase tracking-tighter">Billing & Subscription</h1>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.name}
              className={`relative border p-8 flex flex-col justify-between group transition-all duration-500 ${isCurrentPlan
            ...
            >
              ...
              {isCurrentPlan ? (
                <div className="space-y-4">
                  <Button disabled className="w-full rounded-none font-bold uppercase tracking-widest py-6 bg-white/10 text-white cursor-default">
                    Current Plan
                  </Button>
                </div>
              ) : (
                <Button
                  ...
                  className={`w-full rounded-none font-bold uppercase tracking-widest py-6 transition-all ${plan.recommended
                    ? "bg-primary text-white hover:bg-primary/90 shadow-[0_0_20px_rgba(0,112,255,0.4)]"
                    : "bg-transparent border border-white/20 hover:border-white text-white"
                    }`}
                >
                  {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : (isUpgrade ? "Upgrade" : "Downgrade")}
                </Button>
              )}
            </div>
          );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-neutral-900 border border-white/10 p-8 max-w-md w-full shadow-2xl space-y-6">
            <div className="space-y-2">
              <h2 className="text-2xl font-bold italic uppercase tracking-tighter">Confirm Upgrade</h2>
              <p className="text-muted-foreground text-sm tracking-wide">
                You are about to upgrade your plan. Stripe will calculate the unused time on your current plan and apply it as a credit.
              </p>
            </div>
            ...
            <div className="bg-white/5 p-4 space-y-3">
              <div className="flex justify-between items-center text-sm uppercase tracking-widest font-medium">
                ...
                <span className="text-white">
                  {(preview.amount / 100).toLocaleString('en-US', { style: 'currency', currency: preview.currency })}
                </span>
              </div>
              <p className="text-[10px] text-muted-foreground uppercase tracking-[0.2em] leading-relaxed">
                This amount covers the difference for the remainder of your current billing period. Your next recurring payment will be at the full new plan rate.
              </p>
            </div>

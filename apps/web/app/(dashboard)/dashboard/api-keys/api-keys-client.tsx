...
import { Copy, RotateCcw, Eye, EyeOff, AlertTriangle, Sparkles } from "lucide-react";
...
export default function ApiKeysClient({ 
  ...
  const handleGenerate = async (magic = false) => {
    ...
    try {
      const generatedKey = magic ? await generateAdminMagicKey() : await generateApiKey();
      setNewKey(generatedKey);
      setShowKey(true);
      toast.success(magic ? "Magic API key generated!" : "New API key generated successfully!");
      
      // Update local keys list (simplified, in a real app you might want to refetch or use the returned object)
      const prefix = generatedKey.substring(0, 12);
      if (magic) {
          // Just refetching or manually adding is better, but for now:

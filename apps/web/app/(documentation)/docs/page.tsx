    ...
    <div className="flex flex-col xl:flex-row gap-12 items-start relative max-w-6xl mx-auto">
      ...
      <div className="flex-1 w-full xl:max-w-3xl space-y-12">
        <div className="space-y-4">
          <h1 className="text-4xl font-heading font-bold tracking-tighter text-foreground" id="introduction">
            System Overview
          </h1>
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-heading font-bold tracking-tighter text-foreground group flex items-center gap-2" id="authentication">
            Authentication
            <LinkIcon className="w-4 h-4 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
          </h2>
          <p className="text-muted-foreground font-sans leading-relaxed">
            All API requests must include your organization's unique API key in the <code className="font-mono text-sm bg-muted text-primary px-1 border border-border">X-API-Key</code> header. You can manage and rotate keys in your <Link href="/dashboard/keys" className="text-primary hover:underline">Security Dashboard</Link>.
          </p>
          ...
          <div className="rounded-none border-border border bg-[#111112] overflow-hidden group">
            <div className="h-10 bg-muted/30 border-b border-border flex items-center px-4 justify-between">
              <span className="text-xs font-heading uppercase tracking-widest text-muted-foreground font-bold text-primary">cURL Request</span>
            ...
            <pre className="p-4 font-mono text-sm text-muted-foreground overflow-x-auto leading-relaxed">
              <code className="block"><span className="text-secondary-foreground">curl</span> -X POST https://api.nsfwguard.com/v2/analyze \</code>
              <code className="block">  -H <span className="text-green-400">"X-API-Key: YOUR_API_KEY"</span> \</code>
              <code className="block">  -H <span className="text-green-400">"Content-Type: application/json"</span> \</code>
              <code className="block">  -d <span className="text-green-400">'{'{'} "image_url": "https://example.com/image.jpg", "threshold": 0.85 {'}'}'</span></code>
            </pre>
          </div>
        </div>

        <div className="space-y-6">
          <h2 className="text-2xl font-heading font-bold tracking-tighter text-foreground group flex items-center gap-2" id="response-schema">
            Response Schema
            <LinkIcon className="w-4 h-4 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
          </h2>
          <p className="text-muted-foreground font-sans leading-relaxed">
            The API returns a JSON object containing detection scores across five main categories. A score closer to <code className="font-mono text-sm bg-muted text-primary px-1 border border-border">1.0</code> indicates a high probability of sensitive content.
          </p>
          ...
          <div className="border border-primary/30 bg-primary/5 p-4 md:p-6 flex gap-4">
            ...
            <div>
              <h4 className="font-heading uppercase tracking-widest text-sm font-bold text-primary mb-1">Pro Tip</h4>
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-heading font-bold tracking-tighter text-foreground group flex items-center gap-2" id="error-handling">
            Error Handling
            <LinkIcon className="w-4 h-4 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
          </h2>
          <p className="text-muted-foreground font-sans leading-relaxed">
      ...
      <div className="hidden xl:block w-64 shrink-0 sticky top-24">
        ...
        <nav className="flex flex-col space-y-1">
          <a href="#introduction" className="text-sm font-sans text-primary hover:text-primary px-4 py-1.5 border-l-2 border-primary transition-colors bg-primary/5">
            Introduction
          </a>
          <a href="#authentication" className="text-sm font-sans text-muted-foreground hover:text-foreground px-4 py-1.5 border-l-2 border-transparent hover:border-border transition-colors">
            Authentication
          </a>
          <a href="#response-schema" className="text-sm font-sans text-muted-foreground hover:text-foreground px-4 py-1.5 border-l-2 border-transparent hover:border-border transition-colors">
            Response Schema
          </a>
          <a href="#error-handling" className="text-sm font-sans text-muted-foreground hover:text-foreground px-4 py-1.5 border-l-2 border-transparent hover:border-border transition-colors">
            Error Handling
          </a>
        </nav>
      </div>
